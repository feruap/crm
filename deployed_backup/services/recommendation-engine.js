...
// ─────────────────────────────────────────────
async function getRecommendationsByRules(messageText, customerProfile) {
    const rules = await db_1.db.query(`SELECT cdr.*, array_agg(mp.name) AS product_names
    ...
    for (const rule of rules.rows) {
        ...
        if (rule.client_profile_filter && rule.client_profile_filter.length > 0 && customerProfile?.business_type) {
...
// ─────────────────────────────────────────────
function adaptForProfile(products, profile) {
    const results = [];
    for (const p of products) {
        let confidence = 0.7;
        let reason = '';
        // Boost if product matches the client profile
        if (profile.business_type && p.recommended_profiles.includes(profile.business_type)) {
            confidence += 0.15;
            reason = `Recomendado para ${profile.business_type}`;
        }
        // Adapt recommendation text by profile
        if (profile.business_type === 'laboratorio' || profile.business_type === 'hospital') {
        ...
        if (profile.estimated_monthly_volume === 'alto_201_1000' || profile.estimated_monthly_volume === 'mayoreo_1000_plus') {
...
// ─────────────────────────────────────────────
async function getSemanticRecommendations(messageText, provider, apiKey, limit = 5) {
    const embedding = await (0, ai_service_1.generateEmbedding)(messageText, provider, apiKey);
    const vectorLiteral = `[${embedding.join(',')}]`;
    // Search products by embedding similarity
    const products = await db_1.db.query(`SELECT id, name, diagnostic_category, clinical_indications,
    ...
    const chunks = await db_1.db.query(`SELECT mkc.medical_product_id, mkc.content, mkc.chunk_type,
    ...
    for (const c of chunks.rows) {
        if (seenProducts.has(c.medical_product_id))
            continue;
        if (c.similarity < 0.4)
            continue;
        results.push({
...
// ─────────────────────────────────────────────
async function getCrossSellRecommendations(customerId) {
    // Get products the customer has bought (via WC order items)
    const orders = await db_1.db.query(`SELECT items FROM orders WHERE customer_id = $1 AND status NOT IN ('cancelled', 'refunded', 'failed')
         ORDER BY order_date DESC LIMIT 10`, [customerId]);
    ...
    for (const order of orders.rows) {
        ...
        for (const item of items) {
            if (item.product_id)
                boughtProductIds.add(item.product_id);
        }
    }
    if (boughtProductIds.size === 0)
        return [];
    // Find medical products matching what they bought and get complementary products
    const bought = await db_1.db.query(`SELECT id, name, complementary_product_ids, diagnostic_category
         FROM medical_products
         WHERE wc_product_id = ANY($1) AND is_active = TRUE`, [Array.from(boughtProductIds)]);
    ...
    const comps = await db_1.db.query(`SELECT id, name, diagnostic_category, complementary_product_ids
         FROM medical_products
         WHERE id = ANY($1) AND is_active = TRUE`, [Array.from(complementaryIds)]);
    return comps.rows.map((c) => ({
        product_id: c.id,
        product_name: c.name,
...
async function getCustomerProfile(customerId) {
    const profile = await db_1.db.query(`SELECT business_type, specialty, estimated_monthly_volume, detected_interests
 ...
 */
async function updateCustomerProfile(customerId, data, source = 'ai_detected') {
    await db_1.db.query(`INSERT INTO customer_profiles (customer_id, business_type, specialty, estimated_monthly_volume, detected_interests, source)
         VALUES ($1, $2, $3, $4, $5, $6)
 ...
 */
async function getRecommendations(messageText, customerId, provider, apiKey) {
    ...
    const sorted = Array.from(deduped.values()).sort((a, b) => b.confidence - a.confidence);
    // Return top 5
