...
exports.generateWCCartLink = generateWCCartLink;
exports.buildCustomerWCContext = buildCustomerWCContext;
exports.getCustomerWCOrders = getCustomerWCOrders;
exports.getOrderWithKanbanState = getOrderWithKanbanState;
exports.mapWCStatusToKanban = mapWCStatusToKanban;
exports.requestSKDiscount = requestSKDiscount;
exports.attributeOrderToConversation = attributeOrderToConversation;
exports.getAgentCommissionForOrder = getAgentCommissionForOrder;
exports.getB2BPricing = getB2BPricing;
exports.getAgentKanbanPermissions = getAgentKanbanPermissions;
 ...
 */
function generateWCCartLink(params) {
 ...
 */
async function buildCustomerWCContext(customerEmail) {
    try {
        // In production, this would call WC REST API
        // For now, return minimal context that can be populated from DB
        const dbResult = await db_1.db.query(`SELECT id FROM customers WHERE email = $1 LIMIT 1`, [customerEmail]).catch(() => ({ rows: [] }));
        ...
        const ordersResult = await db_1.db.query(`SELECT id, total_amount, status, created_at
             FROM orders
             WHERE customer_id = $1
             ORDER BY created_at DESC
             LIMIT 10`, [dbResult.rows[0].id]).catch(() => ({ rows: [] }));
 ...
 */
async function getCustomerWCOrders(customerEmail) {
    try {
        // In production: call WC REST API
        // For now: query from CRM database
        const result = await db_1.db.query(`SELECT o.id, o.status, o.total_amount, o.created_at
             FROM orders o
             JOIN customers c ON o.customer_id = c.id
             WHERE c.email = $1
             ORDER BY o.created_at DESC
             LIMIT 20`, [customerEmail]).catch(() => ({ rows: [] }));
 ...
 */
async function getOrderWithKanbanState(orderId) {
    try {
        const result = await db_1.db.query(`SELECT id, status, total_amount, created_at FROM orders WHERE id = $1`, [orderId]).catch(() => ({ rows: [] }));
 ...
 */
function mapWCStatusToKanban(wcStatus) {
 ...
 */
async function requestSKDiscount(params) {
    try {
        ...
        if (discountPercent > maxDiscount) {
            ...
            try {
                await db_1.db.query(`INSERT INTO discount_requests (customer_id, agent_id, discount_percent, reason, status)
                     VALUES ($1, $2, $3, $4, 'pending')
                     ON CONFLICT DO NOTHING`, [customerId, agentId, discountPercent, reason]).catch(() => null);
            }
            catch {
                // Table may not exist
            }
...
async function attributeOrderToConversation(params) {
    try {
        ...
        try {
            await db_1.db.query(`INSERT INTO attribution_chain (customer_id, conversation_id, order_id, campaign_id, order_total)
                 SELECT cu.id, $2, $1, $3, $4
                 FROM customers cu
                 WHERE cu.email = $5
                 ON CONFLICT DO NOTHING`, [orderId, conversation.id, utmCampaign, orderTotal, orderEmail]).catch(() => null);
        }
        catch {
            // Table may not exist
        }
 ...
 */
async function getAgentCommissionForOrder(orderId, agentId) {
    try {
        // Check conversation_commissions table
        const result = await db_1.db.query(`SELECT order_total, commission_rate, commission_amount
             FROM conversation_commissions
             WHERE order_id = $1 AND agent_id = $2`, [orderId, agentId]).catch(() => ({ rows: [] }));
        if (result.rows.length > 0) {
 ...
 */
async function getB2BPricing(productId, customerGroupId) {
    try {
        if (!customerGroupId) {
 ...
 */
function getAgentKanbanPermissions(agentId, agentRole) {
