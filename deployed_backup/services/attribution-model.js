...
exports.attributeOrderRevenue = attributeOrderRevenue;
exports.recalculateAllAttributions = recalculateAllAttributions;
exports.getAttributionConfig = getAttributionConfig;
exports.updateAttributionConfig = updateAttributionConfig;
const db_1 = require("../db");
// ─────────────────────────────────────────────
// Load Config
// ─────────────────────────────────────────────
async function getConfig() {
    const result = await db_1.db.query(`SELECT * FROM attribution_config WHERE is_active = TRUE LIMIT 1`);
...
// ─────────────────────────────────────────────
function calculateFirstTouch(touchpoints) {
...
function calculateLastTouch(touchpoints) {
    const weights = new Map();
    if (touchpoints.length === 0)
        return weights;
    touchpoints.forEach(tp => weights.set(tp.id, 0));
...
function calculateLinear(touchpoints) {
    const weights = new Map();
    if (touchpoints.length === 0)
        return weights;
...
function calculateTimeDecay(touchpoints, halflifeDays, conversionTime) {
    const weights = new Map();
    if (touchpoints.length === 0)
        return weights;
...
function calculatePositionBased(touchpoints, firstWeight, lastWeight) {
    const weights = new Map();
    if (touchpoints.length === 0)
        return weights;
    ...
    touchpoints.forEach((tp, i) => {
        if (i === 0)
            weights.set(tp.id, firstWeight);
        else if (i === touchpoints.length - 1)
            weights.set(tp.id, lastWeight);
        else
            weights.set(tp.id, perMiddle);
    });
    return weights;
}
// ─────────────────────────────────────────────
 ...
 */
async function attributeOrderRevenue(orderId, customerId) {
    const config = await getConfig();
    // Get the order
    const orderResult = await db_1.db.query(`SELECT total_amount, currency, order_date FROM orders WHERE id = $1`, [orderId]);
    if (orderResult.rows.length === 0)
        return { touchpoints: [], model: config.model_type };
    ...
    const touchpoints = await db_1.db.query(`SELECT id, customer_id, platform, campaign_id, ad_id, created_at
         FROM attribution_touchpoints
         WHERE customer_id = $1 AND created_at >= $2 AND created_at <= $3
         ORDER BY created_at ASC`, [customerId, lookbackDate.toISOString(), orderDate.toISOString()]);
    const tps = touchpoints.rows;
    if (tps.length === 0)
        return { touchpoints: [], model: config.model_type };
    ...
    for (const [tpId, weight] of weights.entries()) {
        ...
        await db_1.db.query(`UPDATE attribution_touchpoints
             SET attribution_weight = $1, attributed_revenue = attributed_revenue + $2
             WHERE id = $3`, [weight, revenue, tpId]);
    ...
    if (lastTp.campaign_id) {
        await db_1.db.query(`UPDATE attributions
             SET attributed_revenue = $1, attribution_model = $2, attribution_weight = 1.0
             WHERE order_id = $3 AND campaign_id = $4`, [orderRevenue, config.model_type, orderId, lastTp.campaign_id]);
    }
 ...
 */
async function recalculateAllAttributions() {
    const config = await getConfig();
    // Reset all touchpoint attributed_revenue
    await db_1.db.query(`UPDATE attribution_touchpoints SET attributed_revenue = 0, attribution_weight = 0`);
    await db_1.db.query(`UPDATE attributions SET attributed_revenue = 0, attribution_model = $1`, [config.model_type]);
    // Get all completed orders with a customer
    const orders = await db_1.db.query(`SELECT o.id, o.customer_id
         FROM orders o
...
// ─────────────────────────────────────────────
async function getAttributionConfig() {
    return getConfig();
}
/**
 * Update attribution model configuration.
 */
async function updateAttributionConfig(updates) {
    const result = await db_1.db.query(`UPDATE attribution_config SET
