...
exports.syncWCPrices = syncWCPrices;
exports.handleWCProductWebhook = handleWCProductWebhook;
exports.scheduledPriceSync = scheduledPriceSync;
const db_1 = require("../db");
// ─────────────────────────────────────────────
// WC API Client
// ─────────────────────────────────────────────
async function wcApiRequest(endpoint, params = {}) {
...
// ─────────────────────────────────────────────
function parseWCPresentaciones(product, variations) {
...
async function syncWCPrices() {
    ...
    const crmProducts = await db_1.db.query(`
...
// ─────────────────────────────────────────────
async function handleWCProductWebhook(payload) {
    const wcId = payload.id;
    if (!wcId)
        return;
    console.log(`[WC Webhook] Product updated: WC#${wcId}`);
    // Find matching CRM product
    const match = await db_1.db.query('SELECT id, name FROM medical_products WHERE wc_product_id = $1', [wcId]);
...
// ─────────────────────────────────────────────
async function scheduledPriceSync() {
    try {
