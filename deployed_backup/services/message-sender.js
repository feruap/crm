...
// WhatsApp Cloud API
// ─────────────────────────────────────────────
async function sendWhatsApp(phoneNumberId, accessToken, recipientPhone, text) {
    try {
...
// ─────────────────────────────────────────────
async function sendFacebookMessenger(pageAccessToken, recipientPSID, text) {
    try {
...
// ─────────────────────────────────────────────
async function sendInstagramDirect(pageAccessToken, recipientIGSID, text) {
    try {
        const response = await fetch(`https://graph.facebook.com/v19.0/me/messages`, {
            method: 'POST',
            headers: {
 ...
 */
async function deliverMessage(messageId, conversationId, customerId, channelId, content) {
    try {
        // Get channel config
        const channelResult = await db_1.db.query(`SELECT provider, provider_config FROM channels WHERE id = $1`, [channelId]);
        ...
        const identity = await db_1.db.query(`SELECT provider_id FROM external_identities
             WHERE customer_id = $1 AND provider = $2
             LIMIT 1`, [customerId, channel.provider]);
        ...
        if (result.ok && result.provider_message_id) {
            await db_1.db.query(`UPDATE messages SET provider_message_id = $1 WHERE id = $2`, [result.provider_message_id, messageId]);
        }
        // Log delivery result
        if (!result.ok) {
 ...
 */
async function sendAndSaveMessage(conversationId, channelId, customerId, content, handledBy = 'bot', botAction, botConfidence) {
    // Save to DB first
    const msg = await db_1.db.query(`INSERT INTO messages (conversation_id, channel_id, customer_id, direction, content, handled_by, bot_action, bot_confidence)
         VALUES ($1, $2, $3, 'outbound', $4, $5, $6, $7)
         RETURNING id`, [conversationId, channelId, customerId, content, handledBy, botAction || null, botConfidence || null]);
