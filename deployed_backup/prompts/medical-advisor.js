...
exports.PROFILE_DETECTION_PROMPT = exports.MEDICAL_ADVISOR_BASE_PROMPT = void 0;
exports.buildMedicalPrompt = buildMedicalPrompt;
// ─────────────────────────────────────────────
// Base System Prompt
// ─────────────────────────────────────────────
exports.MEDICAL_ADVISOR_BASE_PROMPT = `Eres un asesor técnico especializado en pruebas de diagnóstico rápido para Botón Médico. Tu rol es ayudar a profesionales de salud a elegir las pruebas diagnósticas adecuadas para sus necesidades clínicas.
 ...
 */
function buildMedicalPrompt(options) {
    const parts = [exports.MEDICAL_ADVISOR_BASE_PROMPT];
...
${options.productCatalog.map(p => {
            ...
            if (p.result_time)
                entry += ` | Resultado: ${p.result_time}`;
            if (p.sample_type)
                entry += ` | Muestra: ${p.sample_type}`;
            if (p.methodology)
                entry += ` | Método: ${p.methodology}`;
            return entry;
        }).join('\n')}`);
    }
...
${options.recommendations.map((r, i) => `${i + 1}. **${r.product_name}** — ${r.reason} (confianza: ${(r.confidence * 100).toFixed(0)}%, fuente: ${r.source})`).join('\n')}
 ...
 */
exports.PROFILE_DETECTION_PROMPT = `Analiza los siguientes mensajes de una conversación y extrae el perfil del cliente.
