...
exports.findBestAnswer = findBestAnswer;
exports.findMedicalContext = findMedicalContext;
exports.generateEmbedding = generateEmbedding;
exports.getAIResponse = getAIResponse;
exports.getMedicalBotResponse = getMedicalBotResponse;
exports.learnFromConversation = learnFromConversation;
exports.recordKnowledgeUse = recordKnowledgeUse;
const db_1 = require("./db");
const medical_advisor_1 = require("./prompts/medical-advisor");
const recommendation_engine_1 = require("./services/recommendation-engine");
...
// ─────────────────────────────────────────────
async function findBestAnswer(question, embedding) {
    const vectorLiteral = `[${embedding.join(',')}]`;
    const result = await db_1.db.query(`SELECT id, answer, confidence_score,
                1 - (embedding <=> $1::vector) AS similarity
         FROM knowledge_base
         ORDER BY embedding <=> $1::vector
         LIMIT 1`, [vectorLiteral]);
    if (result.rows.length === 0)
        return null;
    const row = result.rows[0];
    const confidence = row.similarity * row.confidence_score;
    if (confidence < BOT_CONFIDENCE_THRESHOLD)
        return null;
    return {
        answer: row.answer,
        confidence,
...
// ─────────────────────────────────────────────
async function findMedicalContext(embedding, limit = 3, audienceType, originalQuery) {
    let relevantChunks = [];
    ...
    if (hasRealEmbedding) {
        ...
        try {
            const chunkResult = await db_1.db.query(`SELECT mkc.content, mkc.chunk_type, mp.name AS product_name,
                 ...
                 LIMIT $2`, [vectorLiteral, limit]);
            relevantChunks = chunkResult.rows.filter((r) => r.similarity > 0.3);
        }
        catch (err) {
        ...
        try {
            const productResult = await db_1.db.query(`SELECT id, name, diagnostic_category, analito, result_time,
                 ...
                 ORDER BY embedding <=> $1::vector
                 LIMIT $2`, [vectorLiteral, limit]);
            relevantProducts = productResult.rows.filter((r) => r.similarity > 0.3);
        }
        catch (err) {
    ...
    if (relevantProducts.length === 0) {
        ...
        try {
            ...
            const allProducts = await db_1.db.query(`SELECT id, name, diagnostic_category, analito, result_time,
                        sensitivity, specificity, precio_publico, presentaciones,
                        pitch_medico, pitch_laboratorio, ventaja_vs_lab, roi_medico,
                        porque_agregarlo_lab, objeciones_medico, objeciones_laboratorio,
            ...
            const scored = allProducts.rows.map((p) => {
                let score = 0;
                ...
                for (const term of medTerms) {
                    if (queryLower.includes(term) && searchableText.includes(term))
                        score += 3;
                }
            ...
            relevantProducts = scored.filter((p) => p.similarity > 0).slice(0, limit);
            console.log(`[RAG Keyword] Found ${relevantProducts.length} products: ${relevantProducts.map((p) => p.name).join(', ')}`);
        }
        catch (err) {
    ...
    for (const p of relevantProducts) {
        ...
        if (p.presentaciones && p.presentaciones.length > 0) {
            entry += `\nPresentaciones: ${p.presentaciones.map((pr) => `Caja con ${pr.cantidad} ($${pr.precio})`).join(', ')}`;
        }
        // Cross-sells
        if (p.cross_sells && p.cross_sells.length > 0) {
            entry += `\nPruebas complementarias: ${p.cross_sells.map((cs) => cs.name).join(', ')}`;
        }
        if (p.url_tienda)
            entry += `\nURL: ${p.url_tienda}`;
        parts.push(entry);
...
// ─────────────────────────────────────────────
async function generateEmbedding(text, provider, apiKey) {
...
// ─────────────────────────────────────────────
async function getAIResponse(provider, systemPrompt, userMessage, apiKey) {
...
// ─────────────────────────────────────────────
async function getMedicalBotResponse(conversationId, customerId, messageText, provider, apiKey, baseSystemPrompt) {
    ...
    const recommendations = await (0, recommendation_engine_1.getRecommendations)(messageText, customerId, provider, apiKey);
    // 5. Get customer profile for personalization
    const customerProfile = await (0, recommendation_engine_1.getCustomerProfile)(customerId);
    // 6. Get product catalog for context
    const catalog = await db_1.db.query(`SELECT name, diagnostic_category AS category, clinical_indications AS indications,
    ...
    const medicalPrompt = (0, medical_advisor_1.buildMedicalPrompt)({
...
// ─────────────────────────────────────────────
async function learnFromConversation(conversationId, provider, apiKey) {
    const messages = await db_1.db.query(`SELECT direction, content FROM messages
    ...
    for (let i = 0; i < rows.length - 1; i++) {
        if (rows[i].direction === 'inbound' && rows[i + 1].direction === 'outbound') {
            const question = rows[i].content;
            const answer = rows[i + 1].content;
            if (question.length < 5 || answer.length < 5)
                continue;
            const embedding = await generateEmbedding(question, provider, apiKey);
            const vectorLiteral = `[${embedding.join(',')}]`;
            await db_1.db.query(`INSERT INTO knowledge_base (question, answer, source_conversation_id, embedding)
                 VALUES ($1, $2, $3, $4::vector)
                 ON CONFLICT DO NOTHING`, [question, answer, conversationId, vectorLiteral]);
        }
    }
}
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
async function recordKnowledgeUse(knowledgeId) {
    await db_1.db.query(`UPDATE knowledge_base SET use_count = use_count + 1 WHERE id = $1`, [knowledgeId]);
}
