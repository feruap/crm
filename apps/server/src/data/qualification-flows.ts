...
export interface QualificationStep {
    ...
    question: string;                     // Spanish question to ask
    expected_patterns?: {
        regex?: string[];                 // regex patterns for answer validation
        keywords?: string[];              // keywords to look for
    };
    score_on_match?: number;              // points to add if matched
    next_step?: string;                   // next step ID (or 'complete')
    alternative_step?: string;            // alternative path if no match
}

export const QUALIFICATION_FLOWS = {
 ...
 */
export function getFirstQualificationStep(flowType: string): QualificationStep | null {
 ...
 */
export function getQualificationStepById(flowType: string, stepId: string): QualificationStep | null {
    const flow = (QUALIFICATION_FLOWS as any)[flowType];
    if (!flow || !flow.steps) return null;
    return flow.steps.find((s: QualificationStep) => s.id === stepId) || null;
}
...
export function calculateQualificationScore(answers: QualificationAnswers): {
    total_score: number;
    ...
    if (answers.patient_volume) {
        ...
        if (vol.includes('1000') || vol.includes('mayoreo')) volume_score = 20;
        else if (vol.includes('201') || vol.includes('alto')) volume_score = 15;
        else if (vol.includes('51') || vol.includes('medio')) volume_score = 10;
        else if (vol.includes('1') || vol.includes('bajo')) volume_score = 5;
    }
 ...
 */
export function shouldRouteToSalesAgent(score: { total_score: number; professional_score: number; volume_score: number }): boolean {
    // Route to sales if:
...
export function getRoutingRecommendation(
    score: { total_score: number; professional_score: number; volume_score: number }
): {
    priority: 'hot' | 'warm' | 'cold';
    recommended_agent_role?: 'sales' | 'support';
    next_action: string;
} {
