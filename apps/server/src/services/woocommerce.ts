...
async function wcFetch(endpoint: string, method: string = 'GET', body?: unknown): Promise<unknown> {
 ...
 */
const STATUS_MAP_CRM_TO_WC: Record<string, string> = {
    pending: 'pending',
 ...
 */
async function isRecentSync(externalOrderId: string, direction: string, windowMs: number = 10000): Promise<boolean> {
    const result = await db.query(
        `SELECT id FROM order_sync_log
         WHERE external_order_id = $1
           AND sync_direction = $2
           AND synced_at > NOW() - INTERVAL '${windowMs} milliseconds'
           AND error IS NULL
         LIMIT 1`,
        [externalOrderId, direction]
    );
...
async function logSync(
    ...
    await db.query(
        `INSERT INTO order_sync_log (order_id, external_order_id, previous_status, new_status, source, sync_direction, error)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [orderId, externalOrderId, previousStatus, newStatus, source, direction, error]
    );
}
 ...
 */
export async function pushStatusToWC(
    orderId: number,
    externalOrderId: string,
    newStatus: string,
    previousStatus: string | null
 ...
 */
export async function receiveStatusFromWC(
    externalOrderId: string,
    newStatus: string,
    ...
    try {
        ...
        const order = await db.query(
            `SELECT id, status FROM orders WHERE external_order_id = $1`,
            [externalOrderId]
        );
 ...
 */
export async function getWCOrder(wcOrderId: string): Promise<unknown> {
 ...
 */
export async function getProductStock(productId: number): Promise<{
    in_stock: boolean;
    stock_quantity: number | null;
    stock_status: string;
}> {
    const product = await wcFetch(`/products/${productId}`) as {
        stock_quantity: number | null;
        stock_status: string;
        in_stock: boolean;
    };
...
// ─────────────────────────────────────────────
// Create Order via WC REST API
// ─────────────────────────────────────────────

export interface CreateOrderItem {
    product_id: number;
    quantity: number;
    price?: number; // Override price (for B2B pricing)
}
...
export interface CreateOrderRequest {
    ...
    line_items: CreateOrderItem[];
    coupon_lines?: Array<{ code: string }>;
    meta_data?: Array<{ key: string; value: string }>;
    agent_id?: string; // CRM agent creating the order
}
...
): Promise<{ ok: boolean; wc_order_id?: number; total?: string; error?: string }> {
    try {
        const wcOrderData: Record<string, unknown> = {
            status: 'pending',
        ...
        const result = await wcFetch('/orders', 'POST', wcOrderData) as {
            id: number;
            total: string;
            status: string;
            currency: string;
        ...
        if (request.billing?.email) {
            ...
            if (customer.rows.length > 0) customerId = customer.rows[0].id;
        }
...
// ─────────────────────────────────────────────
// Discount Request (SK Custom Discounts integration)
// ─────────────────────────────────────────────

export interface DiscountRequest {
    order_id?: number;      // WC order ID (if exists)
    customer_email: string;
    product_ids: number[];
    requested_discount_pct: number;
    reason: string;
    agent_id: string;       // CRM agent requesting
}
...
): Promise<{ regular_price: string; sale_price: string; b2b_price?: string }> {
    try {
        const product = await wcFetch(`/products/${productId}`) as {
...
export async function getAgentCommissions(
    ...
    try {
        ...
        if (response.ok) {
            const data = await response.json() as AgentCommission;
            return data;
        }

        // Fallback: query directly from WC customers endpoint (user meta)
        const user = await wcFetch(`/customers/${wcUserId}`) as {
            meta_data: Array<{ key: string; value: string }>;
        };
