...
interface CreateWCOrderRequest {
  line_items: Array<{
    product_id: number;
    quantity: number;
    price?: number;
...
interface CreateWCOrderResult extends SyncResult {
  wc_order_id?: number;
  total?: string;
}
...
export async function receiveStatusFromWC(
  ...
  try {
    // Find order in CRM
    const order = await db.query(`SELECT id, status FROM orders WHERE external_order_id = $1`, [
      externalOrderId,
    ]);

    if (order.rows.length === 0) {
      // Order doesn't exist in CRM yet
    ...
    await db.query(`UPDATE orders SET status = $1 WHERE id = $2`, [newStatus, crmOrder.id]);
 ...
 */
export async function getWCOrder(wcOrderId: number | string): Promise<WCOrder> {
 ...
 */
export async function getProductStock(productId: number | string): Promise<ProductStock> {
 ...
 */
export async function createWCOrder(request: CreateWCOrderRequest): Promise<CreateWCOrderResult> {
  try {
    ...
    const result = await wcFetch<WCOrder>('/orders', 'POST', wcOrderData);
    ...
    await db.query(
      ...
      [String(result.id), customerId, result.total, result.currency || 'MXN', result.status, JSON.stringify(items)]
    );
