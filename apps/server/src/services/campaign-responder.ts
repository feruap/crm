...
export interface MetaReferral {
    source?: string;       // 'ADS' for paid, 'SHORTLINK', 'CUSTOMER_CHAT_PLUGIN'
    type?: string;         // 'OPEN_THREAD'
    ad_id?: string;        // The specific ad that was clicked
    ads_context_data?: {
        ad_title?: string;
        photo_url?: string;
        video_url?: string;
    };
    ref?: string;          // Custom referral parameter
}
 ...
 */
export async function findCampaignMapping(referral: MetaReferral): Promise<CampaignMapping | null> {
...
function guessMediaType(url: string): string {
    ...
    if (lower.match(/\.(pdf)(\?|$)/)) return 'file';
    return 'file';
}
