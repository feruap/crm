...
async function getRecommendationsByRules(
    ...
    for (const rule of rules.rows) {
        ...
        if (rule.client_profile_filter && rule.client_profile_filter.length > 0 && customerProfile?.business_type) {
            if (!rule.client_profile_filter.includes(customerProfile.business_type)) continue;
        }

        // Add each recommended product
        const productIds: number[] = rule.recommended_product_ids || [];
        const productNames: string[] = rule.product_names || [];
...
function adaptForProfile(
    products: Array<{
        id: number; name: string; diagnostic_category: string;
        recommended_profiles: string[]; price_range: string;
...
async function getCrossSellRecommendations(customerId: string): Promise<Recommendation[]> {
    ...
    const sourceProducts: Record<number, string> = {};
    ...
    return comps.rows.map((c: { id: number; name: string; diagnostic_category: string; complementary_product_ids: number[] }) => ({
        product_id: c.id,
        product_name: c.name,
...
export async function updateCustomerProfile(
    customerId: string,
    data: Partial<CustomerProfile & { professional_title?: string; organization_name?: string }>,
...
export async function getRecommendations(
    ...
    const deduped = new Map<number, Recommendation>();
