...
export interface HistoryContext {
    ...
    pendingOrders: Array<{ id: string; status: string; total: string; items: string[] }>;
    daysSinceLastOrder: number | null;
    totalOrders: number;
    lifetimeSpend: number;
    isReorderCandidate: boolean;
    crossSellOpportunities: Array<{ product_name: string; reason: string }>;
    lifecycleStage: 'new' | 'active' | 'at_risk' | 'dormant' | 'churned' | 'prospect';
 ...
 */
export async function analyzeCustomerHistory(customerId: string): Promise<HistoryContext> {
    // Get all orders for this customer
    const orders = await db.query(
        `SELECT id, external_order_id, total_amount, status, items, order_date
    ...
    const pendingOrders = allOrders
        .filter((o: { status: string }) => ['processing', 'on-hold', 'pending'].includes(o.status))
        .map((o: { external_order_id: string; status: string; total_amount: string; items: Array<{ name: string }> }) => ({
            id: o.external_order_id,
            status: o.status,
            total: o.total_amount,
            items: (o.items || []).map((i: { name: string }) => i.name),
        }));
    ...
    // Lifetime spend
    const lifetimeSpend = activeOrders.reduce(
        (sum: number, o: { total_amount: string }) => sum + parseFloat(o.total_amount || '0'), 0
    );

    // Lifecycle stage
    let lifecycleStage: HistoryContext['lifecycleStage'] = 'prospect';
...
async function findCrossSellOpportunities(
    customerId: string,
    orders: Array<{ items: Array<{ product_id: number; name: string }> }>
): Promise<Array<{ product_name: string; reason: string }>> {
    // Extract product IDs the customer has bought
    const boughtIds = new Set<number>();
    for (const order of orders) {
        for (const item of (order.items || [])) {
            if (item.product_id) boughtIds.add(item.product_id);
        }
    }
    ...
    const purchased = await db.query(
        `SELECT id, name, complementary_product_ids
         FROM medical_products
         WHERE wc_product_id = ANY($1) AND is_active = TRUE`,
        [Array.from(boughtIds)]
    );

    const opportunities: Array<{ product_name: string; reason: string }> = [];
    const suggestedIds = new Set<number>();

    for (const p of purchased.rows) {
        for (const compId of (p.complementary_product_ids || [])) {
            if (suggestedIds.has(compId)) continue;

            // Check if customer already bought this complementary product
            const compProduct = await db.query(
                `SELECT id, name, wc_product_id FROM medical_products WHERE id = $1`,
                [compId]
            );

            if (compProduct.rows.length === 0) continue;
            const comp = compProduct.rows[0];

            if (comp.wc_product_id && boughtIds.has(comp.wc_product_id)) continue;
...
function generateGreeting(
    pendingOrders: HistoryContext['pendingOrders'],
    daysSinceLastOrder: number | null,
 ...
 */
export async function recalculateCustomerSegments(): Promise<{ updated: number }> {
    ...
    for (const { customer_id } of customers.rows) {
        ...
        await db.query(
            `INSERT INTO customer_segments (customer_id, segment_type, segment_value, metadata)
             VALUES ($1, 'lifecycle_stage', $2, $3)
             ON CONFLICT (customer_id, segment_type) DO UPDATE SET
                 segment_value = EXCLUDED.segment_value,
                 metadata = EXCLUDED.metadata,
                 last_calculated = NOW()`,
            [customer_id, history.lifecycleStage, JSON.stringify({
        ...
        // Value tier
        let valueTier = 'low';
        if (history.lifetimeSpend >= 100000) valueTier = 'vip';
        else if (history.lifetimeSpend >= 50000) valueTier = 'high';
        else if (history.lifetimeSpend >= 10000) valueTier = 'medium';

        await db.query(
            `INSERT INTO customer_segments (customer_id, segment_type, segment_value)
             VALUES ($1, 'value_tier', $2)
             ON CONFLICT (customer_id, segment_type) DO UPDATE SET
                 segment_value = EXCLUDED.segment_value, last_calculated = NOW()`,
            [customer_id, valueTier]
        );
