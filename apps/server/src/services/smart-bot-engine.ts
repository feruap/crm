...
import { db } from '../db';
import { generateEmbedding, findMedicalContext, getAIResponse } from '../ai.service';
import { getCustomerProfile, getRecommendations } from './recommendation-engine';
...
import { buildCustomerWCContext, getOrderWithKanbanState } from './wc-integration-engine';
...
export interface BotResponse {
  ...
  confidence: number;
  intent_type: string;
  action_type: 'reply' | 'escalate' | 'continue_qualification';
...
interface MedicalContextResult {
  context: string | null;
...
export async function generateCampaignResponse(
  referralData: Record<string, any> | undefined,
  customerId: string,
  conversationId: string
...
export async function runQualificationFlow(
  ...
  try {
    ...
    let state = await db.query(`SELECT * FROM conversation_state WHERE conversation_id = $1`, [conversationId]);
    ...
    if (nextStepId === 'complete') {
      ...
      finalResponse = `Excelente. Hemos recopilado su información. Un especialista en ${
        routing.priority === 'hot' ? 'ventas' : 'soporte'
...
export async function generateMedicalAdvisory(
  ...
  try {
    ...
    const { context: medicalContext, products: matchedProducts, hasGap } = medicalResult;
    ...
    return {
      message:
        'Perdón, hubo un error al procesar tu pregunta médica. Un asesor especializado te contactará en breve.',
      confidence: 0.0,
      intent_type: 'HUMAN_NEEDED',
      action_type: 'escalate',
    };
...
function buildContextualEscalationMessage(
  ...
  const agentType =
    routing.target_type === 'sales_agent'
...
export async function handleIncomingMessage(params: IncomingMessageParams): Promise<BotResponse> {
  const {
    conversationId,
    customerId,
    message,
  ...
  try {
    ...
    let wcContext: WCContext | null = null;

    try {
      const customer = await db
        .query(`SELECT email FROM customers WHERE id = $1`, [customerId])
        .catch(() => ({ rows: [] }));
    ...
    if (intent.intent === 'REORDER') {
      ...
      return {
        message:
          '¡Claro! ¿Qué producto te gustaría comprar? Un asesor te preparará un carrito personalizado con link de pago directo.',
        confidence: 0.85,
        intent_type: 'REORDER',
        action_type: 'reply',
      };
    ...
    // Handle PRICE_REQUEST - show catalog pricing
    ...
    return {
      message:
        '¿Puedo ayudarte con información sobre nuestras pruebas de diagnóstico? O si prefieres, puedo conectarte con un asesor.',
      confidence: 0.7,
      intent_type: 'QUALIFICATION',
      action_type: 'reply',
    };
