...
router.get('/', async (req: Request, res: Response) => {
    ...
    // FIX 2.1: Replace N+1 correlated subqueries with LATERAL joins
    let query = `
        ...
        LEFT JOIN LATERAL (
            SELECT content, created_at, direction, handled_by
    ...
    // Filter by who is handling (bot or human) — uses LATERAL join result
    if (handled_by) {
