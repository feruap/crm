...
class Salesking {
	...
	function __construct() {
	    ...
	    // Add save hook for commission recalculation checkbox
	    add_action('woocommerce_save_order', array($this, 'salesking_recalculate_commission_on_save'), 10, 1);
	...
	function salesking_recalculate_commission_on_save($order_id) {
		if (isset($_POST['salesking_recalculate_commission_edit_order']) && intval($_POST['salesking_recalculate_commission_edit_order']) === 1){
		    $order = wc_get_order($order_id);
		    if (!$order) return;
		...
		if (apply_filters('salesking_reports_data_standard', true)){
			...
			if ($vendor === 'all'){
				...
				if (!empty($subagents)) {
					...
					$team_agents_ids[] = $current_user_id;
					$args['meta_key'] = 'salesking_assigned_agent';
					$args['meta_value'] = $team_agents_ids;
					$args['meta_compare'] = 'IN';
				}
				// if current user is a sub agent and don't have sub agents, use their own id, but if current user is admin or store owner then show all data
				else if (empty($subagents) && !empty($current_user_id) && !current_user_can('manage_woocommerce')){
					$args['meta_key'] = 'salesking_assigned_agent';
