...
(function($){
	...
	function addNewOfferItem(clickedButton){
		...
		if (parseInt(salesking_display_settings.ajax_customers_table) === 0){
			// Clear DataTables saved state if column count changed
			try {
				var actualCols = $('#salesking_dashboard_customers_table thead th').length;
				for (var i = 0; i < localStorage.length; i++) {
					var key = localStorage.key(i);
					if (key && key.indexOf('salesking_dashboard_customers_table') !== -1) {
						var savedState = JSON.parse(localStorage.getItem(key));
						if (savedState && savedState.columns && savedState.columns.length !== actualCols) {
			...
			var oTable = $('#salesking_dashboard_customers_table').DataTable({
	            ...
	            stateLoadCallback: function(settings) {
	                try {
	                    ...
	                    if (data && data.columns && data.columns.length !== settings.aoColumns.length) return null;
	                    return data;
	                } catch(e) { return null; }
	            },
	            dom: 'rt',
		...
		// Security: search requires minimum 3 characters
		...
		// Customer Transfer System: Request transfer
		$('body').on('click', '.salesking_request_transfer', function(){
			var customerid = $(this).val();
			var btn = $(this);
			if (confirm('¿Solicitar transferencia de este cliente?')) {
				btn.prop('disabled', true).find('span').text('Enviando...');
				var datavar = {
					action: 'salesking_request_transfer',
					security: salesking_display_settings.security,
				...
				$.post(salesking_display_settings.ajaxurl, datavar, function(response){
					if (response.success) {
						...
						alert(response.data);
						btn.prop('disabled', false).find('span').text('Solicitar Transferencia');
					}
				});
			}
		});

		// Customer Transfer System: Respond to transfer (accept/reject)
		$('body').on('click', '.salesking_respond_transfer', function(){
			var customerid = $(this).data('customer');
			var requesterid = $(this).data('requester');
			var response_type = $(this).data('response');
			var label = response_type === 'accept' ? 'aceptar' : 'rechazar';
			var btn = $(this);

			if (confirm('¿Seguro que deseas ' + label + ' esta solicitud de transferencia?')) {
				btn.prop('disabled', true);
				var datavar = {
					action: 'salesking_respond_transfer',
					security: salesking_display_settings.security,
					customer_id: customerid,
		...
		// Customer Transfer System: Direct reassign (parent agent)
		$('body').on('click', '.salesking_direct_reassign', function(){
			var customerid = $(this).val();
			var btn = $(this);
			if (confirm('¿Reasignar este cliente de tu subagente a ti?')) {
				btn.prop('disabled', true).find('span').text('Reasignando...');
				var datavar = {
					action: 'salesking_direct_reassign',
					security: salesking_display_settings.security,
					customer_id: customerid,
				};
				$.post(salesking_display_settings.ajaxurl, datavar, function(response){
					if (response.success) {
						...
						alert(response.data);
						btn.prop('disabled', false).find('span').text('Reasignar');
					}
				});
			}
		});

		// Customer Claim: Assign unassigned customer to self
		$('body').on('click', '.salesking_claim_customer', function(){
			var customerid = $(this).val();
			var btn = $(this);
			if (confirm('¿Asignar este cliente a ti?')) {
				btn.prop('disabled', true).find('span').text('Asignando...');
				var datavar = {
				...
				$.post(salesking_display_settings.ajaxurl, datavar, function(response){
					if (response.success) {
						...
						alert(response.data);
						btn.prop('disabled', false).find('span').text('Asignar a mí');
					}
				});
			}
		});
