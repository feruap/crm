...
(function($){
	...
	$( document ).ready(function() {
	    ...
	    $('#salesking_markasreadadmin').on('click', function(){
			var datavar = {
	            action: 'saleskingmarkreadmessagead',
		...
		$('#salesking-activate-license').on('click', function(){
			var datavar = {
	            ...
	            email: $('input[name="salesking_license_email_setting"]').val().trim(),
	            key: $('input[name="salesking_license_key_setting"]').val().trim(),
	            security: salesking.security,
	        };
		...
		// deactivate
		$('#salesking_deactivate_license').on('click', function(){
			if (confirm(salesking.sure_deactivate_license)){
				var datavar = {
		            action: 'saleskingdeactivatelicense',
		            security: salesking.security,
		        };
		        
		        $('#salesking_deactivate_license').notify(salesking.deactivating,{  position: "right",  className: 'info'});
		...
		if (typeof $('#salesking_payout_history_table').DataTable === "function") { 
			...
			var count = jQuery("#salesking_payout_history_table thead tr th").length;
			if (count >= 7){
				var targ = [6,7];
			} else {
				var targ = []; 
			...
			var table = $('#salesking_payout_history_table').DataTable({
				"language": {
				    "url": salesking.datatables_folder+salesking.tables_language_option+'.json'
				},
				oLanguage: {
	            ...
	            dom: salesking.payouthistorydom,
	            columnDefs: [
	                { targets: targ, visible: false}
	              ],
	            order: [[1, 'desc']],
	            buttons: {
	                buttons: [
			...
			setTimeout(function(){
				$('#salesking_payout_history_table tfoot tr:eq(0) th').each( function (i) {
				    var title = $(this).text();
				    $(this).html( '<input type="text" class="salesking_search_table" placeholder="'+salesking.searchtext+'..." />' );
	    ...
	    $('#salesking_save_payment').on('click', function(){
	    	...
	    	if (method !== '' && amount !== ''){
	    		...
	    		alert(salesking.please_set_amount_method);
	    	}
		...
		function updateDynamicRulesOptionsConditions(){
			...
			if (selectedOrders !== 'all' && selectedOrders !== 'all_agent' && selectedOrders !== 'all_earnings' && selectedOrders !== 'all_earnings_singlevalue'){
			...
			if (selectedOrders === 'all_earnings' || selectedOrders === 'all_earnings_singlevalue' || selectedOrders === 'reach_x_number' || selectedOrders === 'first_x_earnings' || selectedOrders === 'first_x_days'){
	   ...
	   */
	   ...
	   function reports_set_chart(){
	   	...
	   	if (firstday !== '' && lastday !== ''){
	   		...
	   		var _skPct = 0;
	   		jQuery('#sk-report-loading').show();
	   		jQuery('#sk-report-loading-fill').css({width:'0%',transition:'none'});
	   		jQuery('#sk-report-loading-pct').text('0%');
	   		var _skTimer = setInterval(function(){
	   			if (_skPct < 60){ _skPct += (Math.random()*6+2); }
	   			else if (_skPct < 85){ _skPct += (Math.random()*2+0.5); }
	   			else if (_skPct < 95){ _skPct += 0.3; }
	   			_skPct = Math.min(_skPct, 95);
	   			jQuery('#sk-report-loading-fill').css({width:Math.round(_skPct)+'%',transition:'width 0.5s ease'});
	   			jQuery('#sk-report-loading-pct').text(Math.round(_skPct)+'%');
	   		}, 600);
	   		...
	   		$.post(ajaxurl, datavar, function(response){
	   			...
	   			// Complete progress to 100% then hide
	   			clearInterval(_skTimer);
	   			jQuery('#sk-report-loading-fill').css({width:'100%',transition:'width 0.2s ease'});
	   			jQuery('#sk-report-loading-pct').text('100%');
	   			setTimeout(function(){ jQuery('#sk-report-loading').fadeOut(400); }, 350);
	   			...
	   			// Update breakdown metrics (catalog, discounts, resultado)
	   			if (data.length > 10) {
	   				$('.salesking_catalog_today').html(data[10]);
	   				$('.salesking_agent_reduction_today').html('-' + data[11]);
	   				$('.salesking_coupon_discount_today').html('-' + data[12]);
	   				$('.salesking_total_descuentos_today').html('-' + data[13]);
	   				$('.salesking_resultado_today').html(data[14]);
	   			}
	   ...
	   // Inject loading bar HTML above the report stats
	   if (jQuery('.salesking_reports_page_wrapper').length) {
	   	jQuery('.salesking_reports_page_wrapper').prepend(
	   		'<div id="sk-report-loading" style="display:none;padding:10px 0 6px;margin-bottom:10px;">'+
	   		'<div style="background:#e0e0e0;border-radius:6px;height:10px;overflow:hidden;margin-bottom:6px;">'+
	   		'<div id="sk-report-loading-fill" style="background:linear-gradient(90deg,#2271b1,#40c4ff);height:100%;width:0%;"></div>'+
	   		'</div>'+
	   		'<p style="text-align:center;font-size:13px;color:#444;margin:0;font-weight:500;">'+
	   		'&#9203; Calculando reporte&hellip; <span id="sk-report-loading-pct" style="color:#2271b1;font-weight:700;min-width:38px;display:inline-block;">0%</span>'+
	   		'</p>'+
	   		'</div>'
	   	);
	   }

	   // Add refresh/reset button next to Quick Select links
	   if (jQuery('.salesking_reports_page_wrapper #salesking_reports_quick_links').length) {
	   	jQuery('#salesking_reports_quick_links').append(
	   		' <a href="' + window.location.pathname + '?page=salesking_reports_teams&refresh=1" ' +
	   		'style="background:#1e88e5;color:#fff;padding:3px 10px;border-radius:4px;font-size:11px;text-decoration:none;margin-left:8px;display:inline-block;vertical-align:middle;">' +
	   		'&#128260; Limpiar Caché</a>'
	   	);
	   }

	   // CSV Download button handler (Reports Teams)
	   $('body').on('click', '#salesking_download_csv_btn', function(){
	   	var firstday = jQuery('.salesking_reports_date_input_from').val();
	   	var lastday  = jQuery('.salesking_reports_date_input_to').val();
	   	var vendor   = jQuery('#salesking_dashboard_days_select').val();
	   	...
	   	// 45-day restriction (client-side)
	   	var today = new Date();
	   	var limitDate = new Date(today.getTime() - (45 * 24 * 60 * 60 * 1000));
	   	var limitStr = limitDate.getFullYear() + '-' +
	   		String(limitDate.getMonth() + 1).padStart(2, '0') + '-' +
	   		String(limitDate.getDate()).padStart(2, '0');

	   	if (firstday < limitStr) {
	   		alert('Solo se permite descargar datos de los últimos 45 días.\nFecha mínima permitida: ' + limitStr);
	   		return;
	   	}
	   	...
	   	// Use XHR for binary blob download
	   	...
	   	xhr.onload = function() {
	   			...
	   			if (contentType.indexOf('application/json') !== -1) {
	   				...
	   				reader.onload = function() {
	   					try {
	   						...
	   						alert(json.data || 'Error al generar el CSV.');
	   					} catch(e) {
	   				...
	   				a.href     = url;
	   				a.download = 'salesking_teams_report_' + firstday + '_' + lastday + '.csv';
	   		...
	   		btn.prop('disabled', false).html('<i class="icon salesking-ni salesking-ni-download" style="margin-right:6px;"></i> 📥 Descargar CSV');
	   	};

	   	xhr.onerror = function() {
	   		alert('Error de conexión al descargar el reporte.');
	   		btn.prop('disabled', false).html('<i class="icon salesking-ni salesking-ni-download" style="margin-right:6px;"></i> 📥 Descargar CSV');
	   	};

	   	var params = 'action=salesking_reports_download_csv&security=' + encodeURIComponent(salesking.security) +
	   		'&vendor=' + encodeURIComponent(vendor) +
	   		'&firstday=' + encodeURIComponent(firstday) +
	   		'&lastday=' + encodeURIComponent(lastday);
