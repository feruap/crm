jQuery(document).ready(function ($) {
    ...
    wp.customize('woo_orders_tracking_settings[timeline_track_info_date_format]', function (value) {
        value.bind(function (newval) {
            preview_datetime(newval, wp.customize('woo_orders_tracking_settings[timeline_track_info_time_format]').get(), wp.customize('woo_orders_tracking_settings[timeline_track_info_datetime_format_locale]').get())
        });
    });
    wp.customize('woo_orders_tracking_settings[timeline_track_info_time_format]', function (value) {
        value.bind(function (newval) {
            preview_datetime(wp.customize('woo_orders_tracking_settings[timeline_track_info_date_format]').get(), newval, wp.customize('woo_orders_tracking_settings[timeline_track_info_datetime_format_locale]').get())
        });
    });
    wp.customize('woo_orders_tracking_settings[timeline_track_info_datetime_format_locale]', function (value) {
        value.bind(function (newval) {
            preview_datetime(wp.customize('woo_orders_tracking_settings[timeline_track_info_date_format]').get(), wp.customize('woo_orders_tracking_settings[timeline_track_info_time_format]').get(), newval)
        });
    });

    function preview_datetime(date_format, time_format, use_locale) {
        $.ajax({
            type: 'POST',
            ...
            data: {
                ...
                use_locale: use_locale ? 1 : '',
                nonce: vi_wot_customize_params.nonce
            },
            success: function (response) {
                if (response.status === 'success') {
                    let results_length = response.results.length;
                    $('.woo-orders-tracking-most-recent-to-oldest').map(function () {
                        for (let i = 0; i < results_length; i++) {
                            $(this).find('.woo-orders-tracking-shortcode-timeline-event-time').eq(i).html(response.results[i]);
                            $(this).find('.woo-orders-tracking-shortcode-timeline-event-content-date').eq(i).html(response.results[i]);
                        }
                    });
                    $('.woo-orders-tracking-oldest-to-most-recent').map(function () {
                        for (let i = 0; i < results_length; i++) {
                            $(this).find('.woo-orders-tracking-shortcode-timeline-event-time').eq(results_length - 1 - i).html(response.results[i]);
                            $(this).find('.woo-orders-tracking-shortcode-timeline-event-content-date').eq(results_length - 1 - i).html(response.results[i]);
                        }
                    })
                }
            },
