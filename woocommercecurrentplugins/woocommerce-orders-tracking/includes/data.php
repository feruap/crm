...
Your sincerely',
		...
		$this->params = wp_parse_args( $woo_orders_tracking_settings, $this->default ) ;
	}
	...
	public function get_params( $name = '', $name_sub = '', $language = '', $default = false ) {
		if ( ! $name ) {
			return apply_filters( 'woo_orders_tracking_settings',$this->params);
		}
		...
		$name_filter = 'woo_orders_tracking_settings-' . $name_t;
		switch ($name){
			case 'key':
				$name_filter = '';
				break;
...
//				'url'                => 'https://www.dpd.co.uk/tracking/trackingSearch.do?search.searchType=0&search.parcelNumber={tracking_number}',
				'url'                => 'https://apis.track.dpd.co.uk/v1/track?postcode=&parcel={tracking_number}',
				'country'            => 'GB',
				'active'             => '',
	...
	public static function search_order_item_by_tracking_number( $tracking_number = '', $order_id = '', $email = '', $carrier_slug = '', $unique = true ) {
		...
		if ( $email ) {
			...
			if ((get_option( 'woocommerce_feature_custom_order_tables_enabled' ) === 'yes' || get_option( 'woocommerce_custom_orders_table_enabled' ) === 'yes' )
			    && get_option( 'woocommerce_custom_orders_table_data_sync_enabled','no' ) === 'no'){
		...
		if ( !empty($matched_ids) ) {
			foreach ( $matched_ids as $id ) {
				...
				$_wot_tracking_number  = trim($order_tmp->get_meta( '_wot_tracking_number', true ));
...
//										break;
	...
	public static function service_carriers_list( $name = '' ) {
		$list = array(
			...
			'17track'      => [
				'name'=>'17Track',
				'status'=>'active',
				'get_key_url'=>'https://api.17track.net/en',
			],
			'cainiao'      => [
				'name'=>'Cainiao',
				'status'=>'active',
			...
			'tracktry'      => [
				'name'=>'Tracktry',
				'status'=>'active',
				'get_key_url'=>'https://www.tracktry.com/signup-en.html',
			],
			'easypost'      => [
				'name'=>'EasyPost',
				'status'=>'active',
				'get_key_url'=>'https://www.easypost.com/signup',
			],
			'aftership'      => [
				'name'=>'AfterShip',
				'status'=>'active',
				'get_key_url'=>'https://help.aftership.com/hc/en-us/articles/115008353227-How-to-generate-AfterShip-API-Key-',
			]
		);
		if ( $name ) {
	...
	public function get_cache_request_time() {
		return HOUR_IN_SECONDS * absint(  self::get_params( 'service_cache_request' ) );
	}
	 ...
	 * @param $order_id
	 * @param $api_error
	 * @param $tracking_db_count
	 */
	public static function add_tracking_to_service( $tracking_number, $carrier_slug, $carrier_name, $order_id, &$api_error, &$tracking_db_count = '' ) {
		$instance = self::get_instance();
			...
			switch ( $service_carrier_type ) {
				case 'trackingmore':
					...
					$track_data       = $trackingMore->create_tracking( $tracking_number, VI_WOOCOMMERCE_ORDERS_TRACKING_TRACKINGMORE::get_carrier_slug_by_name( $carrier_name ),
						$order_id );
					$tracking_db_count = count($tracking_from_db);
					if ( $track_data['status'] === 'success' ) {
						$status = $track_data['data']['status'];
						if ( !$tracking_db_count ) {
							$tracking_db_count = VI_WOOCOMMERCE_ORDERS_TRACKING_TRACKINGMORE_TABLE::insert( $order_id, $tracking_number, $status, $carrier_slug, $carrier_name,
								VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::get_shipping_country_by_order_id( $order_id ), '', '', '' );
						}
					} else {
						if ( $track_data['code'] === 4016 ) {
							if ( !$tracking_db_count) {
								$tracking_db_count = VI_WOOCOMMERCE_ORDERS_TRACKING_TRACKINGMORE_TABLE::insert( $order_id, $tracking_number, '', $carrier_slug, $carrier_name,
									VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::get_shipping_country_by_order_id( $order_id ), '', '', '' );
							}
						} else {
				...
				case '17track':
					...
					$tracking_db_count = count($tracking_from_db);
					if ( !$tracking_db_count) {
						$tracking_db_count = VI_WOOCOMMERCE_ORDERS_TRACKING_TRACK_INFO_TABLE::insert( $tracking_number, $order_id, $carrier_slug, $service_carrier_type, $status, '', '',
				...
				case 'aftership':
					...
					if ( $track_data['status'] === 'success' ) {
						...
						if ( $track_data['code'] === 4003 ) {
							if ( ! $tracking_db_count) {
								$tracking_db_count = VI_WOOCOMMERCE_ORDERS_TRACKING_TRACK_INFO_TABLE::insert( $tracking_number, $order_id, $carrier_slug, $service_carrier_type, $status, $track_info,
									$description, '', '' );
							}
						} else {
							$api_error = $track_data['data'];
				...
				$tracking_db_count = count($tracking_from_db);
					if ( !$tracking_db_count ) {
						$tracking_db_count = VI_WOOCOMMERCE_ORDERS_TRACKING_TRACK_INFO_TABLE::insert( $tracking_number, $order_id, $carrier_slug, $service_carrier_type, '', '', '', '', '' );
					}
					break;
				default:
			}
				...
				case 'trackingmore':
					...
					if ( is_array( $bulk_tracking_service_orders ) && count( $bulk_tracking_service_orders ) ) {
						...
						foreach ( $bulk_tracking_service_orders as $bulk_tracking_service_order ) {
							...
							$search_tracking_number = $wpdb->prepare( $search_tracking_number, $order_id_t, $bulk_tracking_service_order['tracking_code'],// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
								$bulk_tracking_service_order['carrier_id'] );
							$check_exist_tracking   = $wpdb->get_col( $search_tracking_number );// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
		...
		if ( $item_id ) {
			...
			if ( $quantity_index > - 1 ) {
				...
				if ($track_per_quantity !== 'unlimited' && $quantity_index > $quantity - 2 ) {
					$result['error'] = sprintf( esc_html__( 'Maximum quantity_index is %s', 'woocommerce-orders-tracking' ), $quantity );// phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
