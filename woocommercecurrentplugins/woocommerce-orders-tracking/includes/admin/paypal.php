...
class VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_PAYPAL {
	...
	public static function get_api_credentials( $method_id ) {
		$wot           = VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::get_instance();
		...
		if ( $search !== false ) {
			...
			if ( empty( $credentials['id'] ) || empty( $credentials['secret'] ) ) {
				if ( $paypal_sandbox_enable[ $search ] ) {
					$search = array_search( 'default_sandbox', $paypal_method );
				} else {
					$search = array_search( 'default', $paypal_method );
				}
				if ( $search !== false ) {
					$credentials['id']     = $client_id[ $search ];
					$credentials['secret'] = $secret[ $search ];
...
//			'wpg_paypal_pro',
	...
	private static function debug_log( $content ) {
		if ( VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::get_instance()->get_params( 'paypal_debug' ) ) {
